import getConnectedPRsForIssue from '../../src/utils/getConnectedPRsForIssue'
...
describe('getConnectedPRsForIssue', () => {
  ...
  test('resolves zenhub link', () => {
    ...
    expect(result).toEqual([{repo: 'unified', owner: 'openstax', pull_number: '123'}])
  })
  ...
  test('resolves multiple PRs', () => {
    ...
    expect(result).toEqual([
      {repo: 'rex-web', owner: 'openstax', pull_number: '123'},
      {repo: 'rex-web', owner: 'openstax', pull_number: '234'}
    ])
  })
  test('resolves bold', () => {
    const result = getConnectedPRsForIssue({body: '**pull requests**: \n- [ ] openstax/rex-web#123'})
